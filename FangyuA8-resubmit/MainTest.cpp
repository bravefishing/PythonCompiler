 ...
 ***  AWrite a simple code generator for the Pascal subset language**
 ***  Translate  Three Address Code into 8086 assembly language  ***
 *******************************************************************/
#include<iostream>
#include<string>
#include <cstring>
...
int offset=0;
stack<string>tac;
stack<string>sb;//assembly language
...
int main(const int argc,const char*argv[])
{
    ...
    else
    {
        ...
        ofstream outfile;
        ofstream outfile1;
        outfile.open("test8.pas.tac");
        outfile1.open("test8.pas.asm");
	    while(!tac.empty())
        {
            cout<<tac.top()<<endl;
        ...
        cout<<endl;
        cout<<"*********show the Assembly Language for the TAC*******"<<endl;
        while(!sb.empty())
        {
            cout<<sb.top()<<endl;
            outfile1<<sb.top()<<endl;
            sb.pop();
            myoff.pop();
        }
    }
...
void Parser::AssignStat()
{
 ...
 void Parser::Expr(string &code,string&a)
 {
     Relation(code,a);
     code.append(code);
   //tac.push(code);
 ...
 void Parser::Relation(string &code,string&a)
 {
     SimpleExpr(code,a);
  // tac.push(code);
 }

 void Parser::SimpleExpr(string &code,string&a)
 {
     Term(code,a);
     MoreTerm(code,a);//?????????????????????????????????????????????????????
   //tac.push(code);
 ...
 void Parser::Term(string &code,string&a)
 {
     Factor(code,a);//?????????????????????????
     MoreFactor(code,a);//?????????????????????????
  // tac.push(code);
 ...
 void Parser::MoreTerm(string&code,string&a)
 {
 ...
 void Parser::MoreFactor(string&code,string&a)
 {
 ...
 void Parser::Factor(string &code,string&a)
 {
     if(myqueue.front().charClass==idt)
     {
          EntryPtr p;
...
void Parser::ParamsTail()//ParamsTail		->	, idt ParamsTail |, num ParamsTail |e
{
 ...
 void Parser::IOStat()
 {
     if(myqueue.front().charClass==readt)
     {
         InStat();
     }
     if(myqueue.front().charClass==writet||myqueue.front().charClass==writelnt)
     {
         OutStat();
     }
     return;
 ...
 void Parser::OutStat()
 {
     if(myqueue.front().charClass==writet)
     {
         match(writet);
         match(lbrackett);
         WriteList();
     ...
     if(myqueue.front().charClass==writelnt)
     {
         match(writelnt);
         match(lbrackett);
         WriteList();
...
void Parser::WriteToken()
{
    ...
    if(myqueue.front().charClass==literalt)
    {
        match(literalt);
    }
    return;
